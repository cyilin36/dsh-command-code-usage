 ...
 */
export function remainingRatio(resetAt: number | null, periodMs: number, now: number): number {
  if (resetAt === null || !Number.isFinite(resetAt) || !Number.isFinite(periodMs) || periodMs <= 0) return 0
...
export function formatRemaining(resetAt: number | null, now: number): string {
...
export function formatRemainingCompact(resetAt: number | null, now: number): string {
  if (resetAt === null || !Number.isFinite(resetAt)) return '—'
  const diff = resetAt - now
  if (diff <= 0) return '已重置'
  const totalMinutes = Math.floor(diff / 60_000)
...
export function stateHasUsage(state: CommandCodeUsageState | null): state is CommandCodeUsageState & { usage: CommandCodeUsage } {
  return state !== null && state.usage !== undefined
}

/** Short health label for the panel header. */
export function healthLabel(state: CommandCodeUsageState | null): string {
  if (state === null) return '连接中…'
  if (state.health.status === 'ok') return '实时'
  if (stateHasUsage(state)) return '数据过期'
  if (state.health.status === 'unconfigured') return '未配置'
  return '异常'
}
...
export function planLabel(planId: string | null | undefined): string | null {
  ...
  const known: Record<string, string> = { go: 'Go', goat: 'GOAT', pro: 'Pro', max: 'Max' }
...
export async function fetchState(): Promise<CommandCodeUsageState> {
  const response = await fetch(`${PLUGIN_ROUTE_BASE}/state`, { headers: { accept: 'application/json' } })
  if (!response.ok) throw new Error(`state HTTP ${response.status}`)
  return (await response.json()) as CommandCodeUsageState
}

/** Ask the host for an immediate refresh and return the new sample. */
export async function refreshState(): Promise<CommandCodeUsageState> {
...
export type KeyWriteOutcome =
  | { ok: true; state: CommandCodeUsageState }
  | { ok: false; error: string }

/** Store an API key through the host; the value is never echoed back. */
export async function storeKey(value: string): Promise<KeyWriteOutcome> {
...
export async function clearKey(): Promise<KeyWriteOutcome> {
  let response: Response
  try {
...
async function parseKeyOutcome(response: Response): Promise<KeyWriteOutcome> {
  let body: unknown
  try {
  ...
  const record = body as { ok?: unknown; error?: unknown; state?: unknown }
  if (record.ok === true && record.state !== undefined) {
    return { ok: true, state: record.state as CommandCodeUsageState }
  }
  return {
    ok: false,
