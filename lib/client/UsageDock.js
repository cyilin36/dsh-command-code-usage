...
export function UsageDock() {
    ...
    return (_jsxs("div", { className: "ccu-dock", "data-positioned": pos !== null ? 'true' : 'false', style: pos !== null ? { left: pos.x, top: pos.y } : undefined, children: [_jsxs("button", { ref: badgeRef, type: "button", className: "ccu-badge", "data-dragging": dragging ? 'true' : undefined, onClick: handleBadgeClick, onPointerDown: beginDrag, onPointerMove: moveDrag, onPointerUp: finishDrag, onPointerCancel: cancelDrag, title: "Command Code \u7528\u91CF\uFF1A\u5269\u4F59\u989D\u5EA6 / 5h \u6EDA\u52A8 / \u672C\u5468\uFF08\u62D6\u62FD\u53EF\u79FB\u52A8\uFF09", "aria-expanded": open, "aria-label": "Command Code \u7528\u91CF", children: [windows.length > 0 ? (_jsxs(_Fragment, { children: [windows.map((window) => _jsx(Ring, { window: window, now: now, size: 34 }, window.key)), rolling !== undefined && (_jsxs("span", { className: "ccu-badge-countdown", title: "5h \u6EDA\u52A8\u7A97\u53E3\u91CD\u7F6E\u5012\u8BA1\u65F6", children: [_jsx("span", { className: "ccu-badge-countdown-icon", "aria-hidden": "true", children: "\u21BB" }), _jsx("span", { children: formatRemainingCompact(rolling.resetAt, now) })] }))] })) : pool !== undefined ? (_jsxs("span", { className: "ccu-badge-text", children: [formatCreditsCompact(pool.remaining), pool.percentUsed === null ? '' : ` · ${Math.round(pool.percentUsed)}%`] })) : (_jsx("span", { className: "ccu-badge-text", children: "Cmd Code \u2014" })), _jsx("span", { className: "ccu-dot", "data-state": dotState, "aria-hidden": "true" })] }), open && (_jsxs("section", { ref: panelRef, className: "ccu-panel", "data-closing": closing || undefined, "data-below": panelLayout?.below ? 'true' : undefined, "data-left": panelLayout?.useLeft ? 'true' : undefined, style: panelLayout?.useLeft ? { left: panelLayout.leftOffset } : undefined, role: "dialog", "aria-label": "Command Code \u7528\u91CF", onAnimationEnd: handlePanelAnimationEnd, children: [_jsxs("header", { className: "ccu-head", children: [_jsxs("div", { className: "ccu-title", children: [_jsx("span", { className: "ccu-logo", "aria-hidden": "true", children: "\u25C8" }), _jsx("span", { children: "Command Code" }), _jsx("span", { className: "ccu-health", "data-status": state?.health.status ?? 'pending', children: healthLabel(state) })] }), _jsx("div", { className: "ccu-head-actions", children: _jsx("button", { type: "button", className: "ccu-icon-btn", onClick: requestClose, "aria-label": "\u5173\u95ED", children: "\u2715" }) })] }), hasUsage ? (_jsxs(_Fragment, { children: [(state.usage.account !== undefined || plan !== null) && (_jsxs("div", { className: "ccu-account", children: [state.usage.account !== undefined && (_jsx("span", { className: "ccu-account-name", children: state.usage.account.login })), plan !== null && _jsx("span", { className: "ccu-plan", children: plan }), state.usage.subscription?.status !== null
                                        && state.usage.subscription?.status !== undefined && (_jsx("span", { className: "ccu-pool-of", children: state.usage.subscription.status }))] })), pool !== undefined && (_jsxs("div", { className: "ccu-pool", children: [_jsxs("div", { className: "ccu-pool-top", children: [_jsx("span", { className: "ccu-pool-remaining", children: formatCredits(pool.remaining) }), _jsxs("span", { className: "ccu-pool-pct", children: [pool.percentUsed === null
                                                        ? '额度已用 —'
                                                        : `额度已用 ${Math.round(pool.percentUsed)}%`, pool.spent === null ? '' : ` · 本期已花 ${formatCredits(pool.spent)}`] })] }), pool.percentUsed !== null && (_jsx("div", { className: "ccu-bar", role: "img", "aria-label": `额度已用 ${Math.round(pool.percentUsed)}%`, children: _jsx("div", { className: "ccu-bar-fill", "data-tone": percentTone(pool.percentUsed), style: { width: `${Math.min(100, pool.percentUsed)}%` } }) })), _jsxs("div", { className: "ccu-sources", children: [_jsxs("span", { children: ["\u8BA2\u9605 ", formatCredits(pool.monthly)] }), pool.purchased > 0 && _jsxs("span", { children: ["\u8D2D\u4E70 ", formatCredits(pool.purchased)] }), pool.free > 0 && _jsxs("span", { children: ["\u8D60\u9001 ", formatCredits(pool.free)] })] })] })), windows.length > 0 ? (_jsx("div", { className: "ccu-windows", children: windows.map((window) => _jsx(WindowRow, { window: window, now: now }, window.key)) })) : (_jsx("p", { className: "ccu-note", children: "\u8BE5\u5957\u9910\u672A\u8FD4\u56DE 5h / \u672C\u5468\u6EDA\u52A8\u989D\u5EA6\u7A97\u53E3\u3002" })), state.usage.summary !== undefined && (_jsxs("div", { className: "ccu-sources", style: { marginTop: 8 }, children: [_jsxs("span", { children: ["\u672C\u671F\u8BF7\u6C42 ", state.usage.summary.totalCount.toLocaleString('en-US')] }), state.usage.summary.totalTokens !== undefined && (_jsxs("span", { children: ["Token ", formatTokens(state.usage.summary.totalTokens)] }))] })), state.usage.unavailable.length > 0 && (_jsxs("p", { className: "ccu-hint", children: ["\u63A5\u53E3\u672A\u8FD4\u56DE\uFF1A", state.usage.unavailable.join(' / '), "\uFF08\u5176\u4F59\u6570\u636E\u6B63\u5E38\uFF09"] }))] })) : (_jsxs("div", { className: "ccu-note", "data-variant": state === null || state.health.status === 'unconfigured' ? 'info' : 'error', children: [_jsx("p", { className: "ccu-note-title", children: state === null ? '正在连接用量服务…'
...
function WindowRow({ window, now }) {
    ...
    return (_jsxs("div", { className: "ccu-row", children: [_jsx(Ring, { window: window, now: now, size: 46 }), _jsxs("div", { className: "ccu-row-body", children: [_jsxs("div", { className: "ccu-row-name", children: [_jsx("span", { className: "ccu-row-label", children: window.label }), _jsx("span", { className: "ccu-row-sub", children: window.sublabel })] }), _jsx("div", { className: "ccu-row-meta", children: _jsxs("span", { className: "ccu-row-credits", children: [_jsx("span", { className: "ccu-row-dot", "data-tone": tone, "aria-hidden": "true" }), formatCredits(window.used), " / ", formatCredits(window.cap), "\uFF08", Math.round(window.percent), "%\uFF09"] }) }), _jsxs("div", { className: "ccu-row-countdown", "data-expired": window.resetAt !== null && window.resetAt <= now ? 'true' : undefined, children: ["\u91CD\u7F6E\u4E8E ", formatRemaining(window.resetAt, now)] })] })] }));
}
/**
