 ...
 */
export function windowLimitsFromCredits(value) {
    if (!isRecord(value))
        return [];
    const windows = [];
    for (const window of ['fiveHour', 'weekly', 'monthly']) {
...
export function parseSummary(value) {
    ...
    if (totalCost === undefined || totalCount === undefined)
        return null;
 ...
 */
export function assembleUsage(whoami, credits, subscription, summary, reasons = {}) {
 ...
 */
export async function fetchCommandCodeUsage(apiKey, timeoutMs = DEFAULT_TIMEOUT_MS, apiBase = DEFAULT_API_BASE) {
    ...
    const read = async (path, params = {}) => {
        ...
        if (response.status === 401 || response.status === 403) {
            return {
                ok: false,
                error: new Error(response.status === 401
    ...
    if (!whoami.ok)
        throw new Error(`无法读取 Command Code 账户信息：${whoami.error.message}`);
    ...
    for (const result of [credits, subscription, summary]) {
        if (!result.ok && /HTTP 401|HTTP 403/.test(result.error.message))
            throw result.error;
    }
    const usage = assembleUsage(whoami.body, credits.ok ? credits.body : null, subscription.ok ? subscription.body : null, summary.ok ? summary.body : null, {
