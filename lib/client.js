...
function usageWindows(usage) {
  ...
  for (const key of ["fiveHour", "weekly", "monthly"]) {
    ...
    if (window2.cap <= 0)
      continue;
    views.push(buildWindow(key, window2.used, window2.cap, window2.resetAt, false));
  }
  if (credits !== void 0 && !windows.some((w) => w.window === "monthly")) {
    const pool = poolView(usage);
    if (pool !== void 0 && pool.percentUsed !== null) {
