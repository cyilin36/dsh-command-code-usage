...
function usageWindows(usage) {
  ...
  for (const key of ["fiveHour", "weekly"]) {
    ...
    if (window2.cap <= 0)
      continue;
    const percent = window2.used / window2.cap * 100;
    views.push({
      key,
      ...
      used: window2.used,
      cap: window2.cap,
      remaining: Math.max(0, window2.cap - window2.used),
      percent: Math.min(100, Math.max(0, percent)),
      resetAt: window2.resetAt,
