...
实时显示剩余额度与 5h 滚动 / 本周 / 本月三个额度窗口；并附带一份现成的
...
- **徽章（收起态）**：三个迷你双层圆环直接显示 5h 滚动 / 本周 / 本月额度——外环为
  **剩余**额度占比（未使用时为满圆环，随消耗缩短；弧按已用占比着色：
  <60% 绿 / ≥60% 橙 / ≥85% 红），内环为窗口剩余时间（品牌蓝，实时递减）；
  旁附 5h 窗口的秒级倒计时（`↻3h25m`）与状态点。
- **面板（点击展开）**：账户名 + 计划标签；额度池大字（剩余金额）与三个来源
  （订阅 / 购买 / 赠送）；三个窗口行（双层环 + `已用 / 总额（百分比）` + 剩余 +
  重置倒计时）；本期请求数与 Token 数。底部为更新时间、清除 Key、立即刷新。
  标题栏右侧有**极简模式**开关：开启后整个坞折叠为仅一个 5h 滚动环（徽章只留
  一个环，不显示倒计时与状态点；面板只留 5h 一行），该偏好持久化。
...
Command Code 与 OpenCode Go 的额度模型不同，这一点需要说清楚：

| | OpenCode Go | Command Code |
| --- | --- | --- |
| 数据源 | `/usage` 直接返回 rolling / weekly / monthly 三个窗口，各带百分比与重置时间 | `windowLimits` 只有 `fiveHour` 与 `weekly` |
| 「本月」 | 真正的月度窗口 | `credits.monthlyCredits` 是信用池的**来源**（订阅额度），不是窗口 |

因此本插件的本月行按以下优先级取值：

1. **API 若返回 `monthly` 窗口** —— 原样使用（数值、重置时间都照搬）。
2. **否则从额度池推导** —— 已用取本期花费（`usage/summary.totalCost`），总额取
   `剩余 + 已花`，百分比即二者之比，重置时间取计费周期结束
   （`currentPeriodEnd`）。这与参考实现计算「已用百分比」的算法一致。
3. **两者都拿不到就不显示该行** —— 缺消费总额时没有诚实的百分比可言，因此不
   编造。此时面板只显示 5h 与本周两行。

换言之：**本月行在你看到它时总是有真实依据的**，要么来自 API，要么由可信的两个
数推导；拿不到就干脆不出现，而不是显示成 0% 或 100%。
