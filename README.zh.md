...
- **面板（点击展开）**：账户名 + 计划标签；额度池大字（剩余金额）与三个来源余额
  （订阅 / 购买 / 赠送）；两个窗口行（双层环 + `已用 / 总额（百分比）` + 剩余 +
  重置倒计时）；本期已花金额、请求数与 Token 数。底部为更新时间、清除 Key、
  立即刷新。标题栏右侧有**极简模式**开关：开启后整个坞折叠为仅一个 5h 滚动环
  （徽章只留一个环，不显示倒计时与状态点；面板只留 5h 一行），该偏好持久化。
...
参考插件 `dsh-opencode-go-usage` 有本月行，本插件**没有**。这不是遗漏，而是两个
上游的额度模型不同：

| | OpenCode Go | Command Code |
| --- | --- | --- |
| 额度窗口 | `/usage` 返回 rolling / weekly / **monthly** 三个窗口，各带百分比与重置时间 | `windowLimits` **只有** `fiveHour` 与 `weekly` |
| 「本月」 | 真正的月度窗口 | **没有月度窗口** |
| `monthlyCredits` | — | 订阅来源的**剩余余额**（金额），不是带百分比的额度 |

关键在于：`credits.monthlyCredits` 只是一个**余额**，API 从不告诉你「本月总共授予
多少」。没有分母就算不出百分比。

**本插件不做任何月度推算。** 曾经实现过一版「用 `已花 / (余额 + 已花)` 反推月度
百分比」，已删除——那个分母是拼凑出来的，不是 API 说的，用它算出的百分比会让
读数看起来像官方配额而实际是推测值。宁可少一行，也不要一个看似权威的推测数字。

因此面板呈现的全部是 API 直接陈述的事实：

- **两个真实存在的窗口**（5h 滚动 / 本周），带 API 给的已用、额度上限、重置时间
- **额度池余额**：三个来源（订阅 / 购买 / 赠送）各自的余额及合计
- **本期已花**：`usage/summary.totalCost` 原样显示，不参与任何比值计算

如果你需要月度视角，`订阅余额` 本身就是月度授予花剩下的钱，但它不配一个百分比。
